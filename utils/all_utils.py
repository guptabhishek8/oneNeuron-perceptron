...
def prepare_data(df):
    ...
    """
    logging.info(
        "preparing the data by segregating the dependent and independent features")
    X = df.drop("y", axis=1)
    y = df["y"]
    return X, y
...
def save_model(model, filename):
    ...
    Args:
        ...
        filename (str): path to save the trained model to 
    """
...
def save_plot(df, file_name, model):
    ...
    :param file_name: its pathto save the plot
